...
const GRAVITY      = -0.6;
const DAMPING      = 0.999;
const CEIL_BOUNCE  = 0.25;   // bounce off ceiling
...
function createSeed() {
  ...
  return {
    ...
    y:    H + 80,                          // spawn below screen
    vx:   (Math.random() - 0.5) * 3,
    vy:   -(1.5 + Math.random() * 2),      // initial upward velocity
...
function tick() {
  ...
  const sorted = seeds.slice().sort((a, b) => b.y - a.y);
